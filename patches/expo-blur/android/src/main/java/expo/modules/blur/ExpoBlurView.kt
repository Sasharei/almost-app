...
class ExpoBlurView(context: Context, appContext: AppContext) : ExpoView(context, appContext) {
  ...
  override fun onAttachedToWindow() {
    ...
    if (!isBlurViewConfigured) {
      ...
      configureBlurViewSafely()
    }
  }

  private fun configureBlurViewSafely() {
    try {
      configureBlurView()
    } catch (error: Throwable) {
      // Avoid crashing if attach happens while activity is already gone.
      blurMethod = BlurMethod.NONE
      blurView.setBlurEnabled(false)
      this.setBackgroundColor(tint.toBlurEffect(blurRadius))
      Log.w(TAG, "Blur setup failed, falling back to tint-only background", error)
    }
  }

  private fun configureBlurView() {
    val currentActivity = appContext.currentActivity ?: return
