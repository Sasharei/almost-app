...
public object ReactNativeFeatureFlags {
  private fun createDefaultAccessor(): ReactNativeFeatureFlagsAccessor {
    return try {
      val cxxAccessor = ReactNativeFeatureFlagsCxxAccessor()
      // Force-load the JNI lib early so we can fall back safely if it's missing.
      cxxAccessor.commonTestFlag()
      cxxAccessor
    } catch (t: Throwable) {
  ...
  private var accessorProvider: () -> ReactNativeFeatureFlagsAccessor = { createDefaultAccessor() }
