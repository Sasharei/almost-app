...
internal class ShareModule(reactContext: ReactApplicationContext) :
  ...
  /**
   * Open a chooser dialog to send content to other apps.
   *
    ...
    try {
      ...
      if (content.hasKey("url")) {
        ...
        if (!url.isNullOrEmpty()) {
          ...
          mimeType =
              when (uri.scheme) {
                "content" -> reactApplicationContext.contentResolver.getType(uri)
                "file" -> URLConnection.guessContentTypeFromName(uri.lastPathSegment)
                else -> null
