          ...
          if (libraryName != null && cmakeListsPath != null) {
            ...
            addDirectoryString +=
                ...
                if(EXISTS "$nativeFolderPath")
                  ...
                  message(STATUS "[RNGP] Skipping ${libraryName}_autolinked_build: directory not found at $nativeFolderPath")
                endif()
                """
                    .trimIndent()
          }
          if (cxxModuleCMakeListsPath != null) {
            ...
            addDirectoryString +=
                ...
                if(EXISTS "$nativeFolderPath")
                  ...
                  message(STATUS "[RNGP] Skipping ${libraryName}_cxxmodule_autolinked_build: directory not found at $nativeFolderPath")
                endif()
                """
                    .trimIndent()
          }
    ...
    val libraryModules =
        packages.joinToString("\n") { dep ->
          ...
          if (dep.libraryName != null) {
            autolinkedLibraries +=
                """
                if(TARGET $CODEGEN_LIB_PREFIX${dep.libraryName})
                  list(APPEND AUTOLINKED_LIBRARIES $CODEGEN_LIB_PREFIX${dep.libraryName})
                endif()
                """
                    .trimIndent()
          }
          if (dep.cxxModuleCMakeListsModuleName != null) {
            if (autolinkedLibraries.isNotEmpty()) {
              autolinkedLibraries += "\n"
            }
            autolinkedLibraries +=
                """
                if(TARGET ${dep.cxxModuleCMakeListsModuleName})
                  list(APPEND AUTOLINKED_LIBRARIES ${dep.cxxModuleCMakeListsModuleName})
                endif()
                """
                    .trimIndent()
          }
